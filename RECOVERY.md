# RECOVERY.md — fixing Flyt after YouTube changes something

Flyt renders its own UI from YouTube's **data** (`ytInitialData`, InnerTube
...
**recovery runbook**. All `file:line` refer to `flyt.user.js` (currently
...
| Dismiss cross never appears, or clicking it doesn't remove the card (`dismiss-cross`) | The feedback-token paths, the row **icon names** (`HIDE`/`REMOVE`), or `feedbackResponses[].isProcessed` as the success signal | `extractFeedback`/`lockupFeedback`/`undoTokenFrom`, `sendFeedback` | §3; capture a signed-in `browse` response and diff the card menu — signed-OUT payloads carry no feedback tokens at all, so an absent cross when logged out is correct, not a regression |
| Quick Unfollow missing from the card cluster (`dismiss-cross`) | `browseEndpoint.browseId` gone from the byline endpoint (so `channelIdFrom` returns null), or the guide list is empty/failed so `subscribedByGuide` answers `null` | `channelIdFrom`/`getChannelId`, `subscribedByGuide` | §3; the button is deliberately absent unless the guide says `true` — absent while the guide is still loading is correct. A missing id is the real regression |
...
Everything Flyt depends on in YouTube. A change to any of these can break it.
...
| `feedback` (`feedbackTokens:[token], isFeedbackTokenUnencrypted:false, shouldMerge:false`) | `sendFeedback`, near the card hover buttons | card cluster ("Not interested" / "Don't recommend channel" / Undo) | `dismiss-cross` (endpoint mocked; the real signed-in call is UNGUARDED) |
| `subscription/unsubscribe` · `/subscribe` (`channelIds:[UC id], params`) — **also** driven by the card cluster's quick Unfollow, not just the channel/watch buttons | `qaClick(unfQuick, …)` | quick Unfollow + its Undo | `dismiss-cross` (endpoint mocked, guide cache seeded via `itube-guide-cache`) |
...
- **Dismissal tokens** (the cross on each card): `videoRenderer.menu.menuRenderer.items[].menuServiceItemRenderer.serviceEndpoint.feedbackEndpoint.feedbackToken` and, on lockups, `…menuButton.buttonViewModel.onTap.innertubeCommand.showSheetCommand.panelLoadingStrategy.inlineContent.sheetViewModel.content.listViewModel.listItems[].listItemViewModel.rendererContext.commandContext.onTap.innertubeCommand.feedbackEndpoint.feedbackToken` — read by `extractFeedback`/`lockupFeedback`. The two rows are picked by **icon name** (`icon.iconType` / `leadingImage.sources[0].clientResource.imageName`): `HIDE` = this video, `REMOVE` = this channel. **Never match the row text** — it is localized — and **never match on "has a feedbackEndpoint"** either: History's "Remove from watch history" row is also a `feedbackEndpoint` (icon `DELETE`), so a type-keyed extractor would hang a "don't recommend" cross on History that deletes watch history instead. Undo token: `feedbackEndpoint.actions[].replaceEnclosingAction.item.notificationMultiActionRenderer.buttons[].buttonRenderer.serviceEndpoint.undoFeedbackEndpoint.undoToken` (`undoTokenFrom`); if it moves, Undo silently stops being offered while the dismiss itself keeps working.
...
`playVideo`/`pauseVideo` (both only from `reconcilePlayback`, the single
playback-intent reconciler — grep it; nothing else drives play/pause),
`setPlaybackRate` (8136),
...
4. **Re-point, don't restyle.** Flyt renders its own DOM; you are only changing which *data path* feeds it. Do not start reading YouTube's rendered DOM instead.
...
5. **`ytd-app` must exist** for `yt-navigate` to boot YouTube's router and build `#movie_player`; without it Flyt falls back to full page loads.
...
8. **No code comments in `flyt.user.js`** — it must read as human-written. (Comments in `tests/**` are expected.)
...
11. **`window.CSS` gotcha:** never reference the bare identifier `CSS` anywhere in `flyt.user.js` — the file declares a top-level `const CSS` (the CSS-in-JS stylesheet, ~331) later in the same IIFE scope, so an earlier bare reference hits its temporal dead zone and throws at runtime. `node --check` does not catch this. Always use `window.CSS`.
12. **`@grant none` is load-bearing.** Flyt reads `window.ytInitialData`/`window.ytcfg`/`#movie_player` directly in the page's own JS context. Any `@grant` other than `none` (even just declaring one `GM_*` grant) switches Tampermonkey/Userscripts.app into a sandboxed/isolated world where the script no longer shares `window` with the page, and every one of these reads breaks.
