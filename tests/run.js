...
const PAGES = {
  ...
  // A route Flyt deliberately does not implement: it must render the
  // "isn't available in Flyt yet" card and STILL keep ytd-app hidden.
...
function parseArgs(argv) {
  // Four lanes by default: the work is network-bound on cold youtube.com
  // loads, so more lanes than cores still pays, but past ~4 the live site
  // starts being the bottleneck and the runs get noisier rather than faster.
  // `--jobs=1` restores the fully sequential run for bisecting a flake.
  const args = { page: null, check: null, update: false, force: false, selftest: false, jobs: Number(process.env.FLYT_JOBS) || 4 };
  for (const a of argv) {
    if (a.startsWith('--jobs=')) args.jobs = Math.max(1, Number(a.slice('--jobs='.length)) || 1);
...
const FUNCTIONAL_ENTRIES = [
  ...
  { subname: 'sponsorblock', serial: true, label: 'sponsorblock auto-skip (mocked)', page: 'sponsorblock', check: 'functional', errCheck: 'sponsorblock', arg: 'browser', run: (browser) => checkSponsorBlock(browser) },
  ...
  { subname: 'toggle', label: 'Flyt enable/disable toggle', page: 'toggle', check: 'functional', errCheck: 'itube-toggle', arg: 'browser', run: (browser) => checkItubeToggle(browser) },
  ...
  { subname: 'space-toggle', label: 'Space play/pause after button click (coalesced togglePlayback)', page: 'space-toggle', check: 'functional', errCheck: 'space-toggle', arg: 'browser', run: (browser) => checkSpaceToggle(browser) },
  ...
  { subname: 'transcript-default-off', label: 'transcript off by default', page: 'transcript', check: 'functional', errCheck: 'transcript-default-off', arg: 'browser', run: (browser) => checkTranscriptOffByDefault(browser) },
  ...
  { subname: 'mini-expand-seamless', serial: true, label: 'mini-player expand (seamless)', page: 'mini-expand-seamless', check: 'functional', errCheck: 'mini-expand-seamless', arg: 'browser', run: (browser) => checkMiniExpandSeamless(browser) },
  ...
  { subname: 'commentcomposer', label: 'comment composer (hidden + sign-in prompt when logged out)', page: 'commentcomposer', check: 'functional', errCheck: 'comment-composer-signed-out', arg: 'browser', run: (browser) => checkCommentComposerSignedOut(browser) },
  { subname: 'railtoggle', label: 'sidebar collapse toggle (persisted, viewport wins below the breakpoint)', page: 'railtoggle', check: 'functional', errCheck: 'rail-toggle', arg: 'browser', run: (browser) => checkRailToggle(browser) },
  { subname: 'titleowner', label: 'tab title survives ytd-app overwriting it', page: 'titleowner', check: 'functional', errCheck: 'title-ownership', arg: 'browser', run: (browser) => checkTitleOwnership(browser) },
  { subname: 'toolschevron', label: 'Tools chevron points at the menu and reverses', page: 'toolschevron', check: 'functional', errCheck: 'tools-chevron', arg: 'browser', run: (browser) => checkToolsChevron(browser) },
  { subname: 'commentshowmore', label: 'comment "Show more" appears on clamped comments', page: 'commentshowmore', check: 'functional', errCheck: 'comment-showmore', arg: 'browser', run: (browser) => checkCommentShowMore(browser) },
  ...
  { subname: 'native-passthrough', label: 'native routes (/account etc.) pass through to YouTube', page: 'home', check: 'functional', errCheck: 'native-route-passthrough', arg: 'browser', run: (browser) => checkNativeRoutePassthrough(browser) },
  { subname: 'rail-alignment', label: 'collapsed rail centers brand/avatar/nav', page: 'home', check: 'functional', errCheck: 'rail-alignment', arg: 'browser', run: (browser) => checkRailAlignment(browser) },
  ...
  { subname: 'abloop', serial: true, label: 'A-B repeat loop', page: 'abloop', check: 'functional', errCheck: 'ab-loop', arg: 'browser', run: (browser) => checkAbLoop(browser) },
  { subname: 'coldload', serial: true, label: 'cold-load skeleton (https://www.youtube.com/watch?v=aircAruvnKk)', page: 'coldload', check: 'skeleton', errCheck: 'cold-load-skeleton', arg: 'context', run: (context) => checkColdLoadSkeleton(context) },
  ...
  { subname: 'bootloader-cold-watch', serial: true, label: 'boot loader: cold watch load', page: 'bootloader', check: 'cold-watch', errCheck: 'boot-loader-cold-watch', arg: 'context', run: (context) => checkBootLoaderColdLoad(context) },
  { subname: 'bootloader-cold-feed', serial: true, label: 'boot loader: cold home load', page: 'bootloader', check: 'cold-feed', errCheck: 'boot-loader-cold-feed', arg: 'context', run: (context) => checkBootLoaderFeedColdLoad(context) },
  { subname: 'yt-dormancy', serial: true, label: 'parked ytd-app goes dormant off-watch without breaking decode', page: 'ytdormancy', check: 'functional', errCheck: 'yt-dormancy', arg: 'browser', run: (browser) => checkYtDormancy(browser) },
  { subname: 'boot-clears', serial: true, label: 'boot overlay clears on every route (not by the 8s fallback)', page: 'bootclears', check: 'functional', errCheck: 'boot-clears-per-route', arg: 'browser', run: (browser) => checkBootClearsPerRoute(browser) },
  { subname: 'bootloader-reduced-motion', serial: true, label: 'boot loader: reduced motion', page: 'bootloader', check: 'reduced-motion', errCheck: 'boot-loader-reduced-motion', arg: 'context', run: (context) => checkBootLoaderReducedMotion(context) },
  ...
  // runWatchFunctional.
  { subname: 'autoplaynext', serial: true, label: 'autoplay to next video', page: 'autoplaynext', check: 'functional', errCheck: 'autoplay-next', arg: 'browser', run: (browser) => checkAutoplayNext(browser) },
  ...
  { subname: 'dismiss-cross', label: 'card action cluster (not interested / channel / unfollow, mocked)', page: 'dismiss-cross', check: 'functional', errCheck: 'dismiss-cross', arg: 'browser', run: (browser) => checkDismissCross(browser) },
  { subname: 'theme', label: 'light theme (dark default, aurora kept, player stays dark)', page: 'theme', check: 'functional', errCheck: 'theme', arg: 'browser', run: (browser) => checkTheme(browser) },
  { subname: 'rank', label: 'home feed re-ranking (permutation, channel balance, off=identity, search untouched)', page: 'rank', check: 'functional', errCheck: 'rank', arg: 'browser', run: (browser) => checkFeedRanking(browser) },
  { subname: 'leaks', serial: true, label: 'no signal-less listener or node growth across watch<->home round trips', page: 'leaks', check: 'functional', errCheck: 'leaks', arg: 'browser', run: (browser) => checkLeaks(browser) },
  { subname: 'splash', serial: true, label: 'boot splash (real mark, wordmark, fade held to completion)', page: 'splash', check: 'functional', errCheck: 'splash', arg: 'browser', run: (browser) => checkBootSplash(browser) },
  { subname: 'seekrail', label: 'seek rail paints thin but grabs wide (clip survives the fill repaint)', page: 'seekrail', check: 'functional', errCheck: 'seekrail', arg: 'browser', run: (browser) => checkSeekRail(browser) },
  { subname: 'stylesheet', label: 'the Flyt stylesheet actually parsed (const CSS not truncated to NaN)', page: 'stylesheet', check: 'functional', errCheck: 'stylesheet', arg: 'browser', run: (browser) => checkStylesheetApplied(browser) },
  { subname: 'pending', label: 'confirm-first pending ring (still outline, travelling light, off under reduced motion)', page: 'pending', check: 'functional', errCheck: 'pending', arg: 'browser', run: (browser) => checkPendingRing(browser) },
  { subname: 'ctxmenu', label: 'player context menu (youtu.be copy link, no ?si, no playback toggle)', page: 'ctxmenu', check: 'functional', errCheck: 'ctxmenu', arg: 'browser', run: (browser) => checkPlayerContextMenu(browser) },
...
async function runFunctionalEntry(entry, { browser, context, table, sink }) {
  const out = sink || console.log;
  const err = sink || console.error;
  out(`\n--- ${entry.label} ---`);
  ...
  try {
    ...
    err(`  ERROR running the ${entry.subname} check: ${e.stack || e}`);
    result = [{ check: entry.errCheck, detail: String(e.message || e).split('\n')[0] }];
  }
  ...
  // Seconds per entry, so the slow ones are visible instead of inferred. A
  // suite this size gets slow one silent waitForTimeout at a time.
  const secs = (Date.now() - started) / 1000;
  table.push({ page: entry.page, check: entry.check, status, count: violations.length, secs });
  const countSuffix = violations.length ? ` (${violations.length} violation${violations.length === 1 ? '' : 's'})` : '';
  const detailSuffix = detail ? ` — ${detail}` : '';
  out(`  ${entry.label}: ${status}${countSuffix}${detailSuffix}`);
  for (const v of violations) out(`    page=${entry.page} ${fmt(v)}`);
  return status === 'FAIL';
}

// The own-context entries each build their own browser context, so they are
// independent and can overlap. Wall-clock is dominated by cold youtube.com
// loads, not by CPU, which is what makes overlapping them pay.
//
// `serial: true` opts an entry OUT. Those are the checks that assert against a
// wall-clock budget — how fast the boot overlay clears, whether playback
// advanced in a window — and a machine running four WebKit contexts is a
// different machine from the one those budgets were written for. Running them
// concurrently would not make them wrong, it would make them flaky, which is
// worse. They run sequentially afterwards, exactly as before.
async function runFunctionalEntries(entries, { browser, context, table }, jobs) {
  const parallel = entries.filter((e) => e.arg === 'browser' && !e.serial);
  const serial = entries.filter((e) => !(e.arg === 'browser' && !e.serial));
  let anyFail = false;

  const lanes = Math.max(1, Math.min(jobs, parallel.length));
  if (parallel.length) {
    console.log(`\n=== ${parallel.length} independent checks, ${lanes} at a time ===`);
    let next = 0;
    // Each lane collects into its own slot so the summary table stays in
    // declaration order however the completions interleave — a table that
    // reshuffles per run cannot be diffed against the previous one.
    ...
    const worker = async () => {
      for (;;) {
        ...
        const lines = [];
        const failed = await runFunctionalEntry(parallel[i], { browser, context, table: rows[i], sink: (l) => lines.push(l) });
        // Printed as one block on completion: interleaved lines from four
        // concurrent checks are unreadable, and a log nobody reads is the
        // same as no log.
...
async function runPageChecks(context, pageName, url, { checkFilter, update, force }) {
  ...
  if (want('functional')) {
    ...
    // Runs once (on home): clicking a route Flyt doesn't implement must be a
  ...
  if (want('errors')) {
    const violations = [];
    // TWO categories, two rules. The single deny-list of URL substrings this
    // replaces could never be complete: it went red four separate times on
    // endpoints nobody had thought of (/generate_204, gstatic icon sprites,
    // /api/timedtext, youtubei next+player), each time needing another
    // alternative bolted on, and each time after a full 10-minute run.
    //
    // (1) Uncaught JS exceptions — what this check is actually for: a throw out
    //     of Flyt's own code. Two of YouTube's own are excluded by name: its
    //     captions race (`this._player.getOption(…,'tracklist')` before the
    //     module exists — ours is the optional-chained, try/caught
    //     `getOption?.('captions','track')`), and ResizeObserver loop warnings,
    //     which cannot be Flyt's because it constructs no ResizeObserver.
    //
    // (2) Resource-load failures ("Fetch API cannot load …", "XMLHttpRequest
    //     cannot load …"). WebKit routes these through the page-error channel
    //     for third-party requests AND for merely cancelled ones, whether or
    //     not the JS rejection was handled — which is why Flyt's own
    //     deliberately-aborted SponsorBlock fetch used to show up here. And
    //     every fetch flyt.user.js makes IS handled; all five sites:
    //       innertube()            try/catch, returns null
    //       channel /about         inside a try
    //       transcript timedtext   .catch(() => null)
    //       SponsorBlock           abort signal inside a try
    //       fetchDislikes          abort timeout inside a try
    //     So one of these messages can never represent an unhandled Flyt
    //     error. They are counted and printed for a human, not failed. A real
    //     InnerTube break is caught far more loudly elsewhere — no rows on
    //     search, no comments, empty feeds — which is the guard for it.
    //     If a fetch is ever added without a handler, that is what breaks this
    //     reasoning; the list above is the thing to re-verify.
    const JS_ERROR_NOT_OURS = /this\._player\.getOption|ResizeObserver loop completed/;
    const RESOURCE_LOAD_FAILURE = /^(?:Fetch API|XMLHttpRequest) cannot load /;
    const resourceFailures = [];
    for (const err of errors.pageErrors) {
      const first = err.split('\n')[0];
      if (RESOURCE_LOAD_FAILURE.test(first)) { resourceFailures.push(first); continue; }
      if (JS_ERROR_NOT_OURS.test(err)) continue;
      violations.push({ check: 'no-page-errors', detail: first });
    }
    if (resourceFailures.length) {
      const byHost = {};
      for (const f of resourceFailures) {
        const m = /cannot load (?:https?:\/\/)?([^/\s]+)/.exec(f);
        const host = m ? m[1] : 'unknown';
        byHost[host] = (byHost[host] || 0) + 1;
      }
      console.log(`  ${pageName} / errors: ignored ${resourceFailures.length} resource-load failure(s) `
        + `(third-party or cancelled, never an unhandled Flyt rejection): `
        + Object.entries(byHost).map(([h, n]) => `${h} x${n}`).join(', '));
    }
...
async function main() {
  ...
  if (!args.page) {
    const selected = FUNCTIONAL_ENTRIES.filter(
      (e) => !(args.check && args.check !== 'functional' && args.check !== e.subname)
    );
    if (selected.length && await runFunctionalEntries(selected, { browser, context, table }, args.jobs)) anyFail = true;
  }
  ...
  console.log('\n=== SUMMARY ===');
  console.log('page'.padEnd(10) + 'check'.padEnd(14) + 'status'.padEnd(8) + 'secs');
  for (const row of table) {
    console.log(row.page.padEnd(10) + row.check.padEnd(14) + row.status.padEnd(8) + (row.secs ? row.secs.toFixed(1) : ''));
  }
