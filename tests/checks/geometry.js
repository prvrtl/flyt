...
async function checkGeometry(browser) {
  ...
  try {
    ...
    const list = await page.evaluate(() => {
      ...
      return {
        // `rgt` is the CLIENT right edge, not the border-box right edge: on a
        // scrolling pane the border box also spans the scrollbar lane (8px,
        // fixed by the ::-webkit-scrollbar rule so geometry is deterministic
        // across engines). Measuring the right gutter across that lane
        // reported a 16-vs-24 "asymmetry" on every page long enough to
        // scroll, while .content's paddings were symmetric 24/24 the whole
        // time — the lane is reserved space, not visual padding.
        content: { l: content.getBoundingClientRect().left, rgt: content.getBoundingClientRect().left + content.clientWidth },
  ...
  try {
    ...
    const w = await wp.evaluate(() => {
      ...
      return {
        ...
        // The SECONDARY controls only. .watch-stats is quiet meta text,
        // .watch-actions-spacer is the flex gap that pushes them right, and
        // .watch-likes is deliberately a different scale — the reaction
        // circles are sized by how often each is used, so holding them to the
        // buttons' 34px would be asserting the design away.
        actionBtns: [...document.querySelectorAll('#itube .watch-actions > .watch-action-btn')].filter((el) => el.getBoundingClientRect().width > 0).map((el) => {
          const r = el.getBoundingClientRect();
          return { name: el.className.split(' ')[0] || el.tagName, t: r.top, h: r.height };
        }),
        // The reaction cluster shares the row's CENTRE line instead of its
        // height, and the like circle is the larger of the two on purpose.
        reactions: (() => {
          const wrap = document.querySelector('#itube .watch-likes');
          const like = document.querySelector('#itube .watch-like-btn svg');
          const dis = document.querySelector('#itube .watch-dislike-btn svg');
          if (!wrap || !like || !dis) return null;
          const w = wrap.getBoundingClientRect();
          const l = like.getBoundingClientRect();
          const d = dis.getBoundingClientRect();
          return { midY: w.top + w.height / 2, likeSize: l.width, dislikeSize: d.width,
            likeRound: l.width === l.height, dislikeRound: d.width === d.height };
        })(),
      };
    });
    ...
    if (w.watchRight && w.railTabs && w.firstRc) {
      ...
      // The title, not the stage, is the left column's first element — the
      // video info moved above the video. Anchoring this to the stage silently
      // encoded the old order and would pass again if the two ever swapped back.
      if (w.title && Math.abs(w.watchRight.t - w.title.t) > TOL) {
        report('geometry-watch-column-tops', `title top ${w.title.t.toFixed(1)} vs rail top ${w.watchRight.t.toFixed(1)} — the two columns must start level`);
      }
    }
    ...
    // Everything in the watch column has to keep the VIDEO's edges, and at the
    // default 1440x900 that is free: the column width caps the stage, so the
    // stage is the column and anything full-width matches it by accident. Only
    // where the stage's HEIGHT cap binds — a wide, short window, and more so
    // with the rail collapsed — do the header block and the actions strip get
    // the chance to run wider than the video. They did: the reactions hung off
    // its left edge and Tools off its right.
    for (const [label, size] of [['wide', { width: 1920, height: 1000 }], ['wide-short', { width: 1728, height: 900 }]]) {
      for (const railCollapsed of [false, true]) {
        await wp.setViewportSize(size);
        if (railCollapsed) {
          await wp.evaluate(() => {
        ...
        const edges = await wp.evaluate(() => {
          const e = (sel) => { const el = document.querySelector(sel); if (!el) return null; const r = el.getBoundingClientRect(); return { l: Math.round(r.left), rgt: Math.round(r.right) }; };
          return { stage: e('#itube-stage'), head: e('#itube .watch-headblock'), below: e('#itube .watch-below') };
        });
        if (edges.stage && edges.head && edges.below) {
          const where = `${label} ${size.width}x${size.height} rail ${railCollapsed ? 'collapsed' : 'expanded'}`;
          if (edges.head.l !== edges.stage.l || edges.head.rgt !== edges.stage.rgt) {
            report('geometry-headblock-matches-stage', `at ${where} the header block [${edges.head.l},${edges.head.rgt}] does not match the video [${edges.stage.l},${edges.stage.rgt}]`);
          }
          if (edges.below.l !== edges.stage.l || edges.below.rgt !== edges.stage.rgt) {
            report('geometry-actions-match-stage', `at ${where} the actions strip [${edges.below.l},${edges.below.rgt}] does not match the video [${edges.stage.l},${edges.stage.rgt}]`);
          }
        }
        if (railCollapsed) {
          await wp.evaluate(() => {
    ...
    if (w.reactions && w.actionBtns.length) {
      const btnMid = w.actionBtns[0].t + w.actionBtns[0].h / 2;
      if (Math.abs(w.reactions.midY - btnMid) > TOL) {
        report('geometry-reactions-centred', `the reaction cluster's centre (${w.reactions.midY.toFixed(1)}) is off the actions row's centre (${btnMid.toFixed(1)}) — different sizes are fine, a different centre line is not`);
      }
      if (!(w.reactions.likeSize > w.reactions.dislikeSize)) {
        report('geometry-like-bigger', `the like circle (${w.reactions.likeSize}px) must be larger than the dislike circle (${w.reactions.dislikeSize}px) — the size difference is the hierarchy`);
      }
      if (!w.reactions.likeRound || !w.reactions.dislikeRound) {
        report('geometry-reactions-round', `both reaction targets must be square-bounded to render as circles, got like ${w.reactions.likeSize} and dislike ${w.reactions.dislikeSize}`);
      }
    }
