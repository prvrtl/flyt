...
function layoutInPage() {
  ...
  for (const el of all) {
    ...
    // Screen-reader-only text is not visible text. The standard technique
    // clips a 1x1 box rather than using display:none — precisely so the label
    // STAYS in the accessibility tree — so it survives the checks above and
    // then gets measured for contrast and spacing it can never satisfy. The
    // watch page's follow control has one: an icon button whose accessible
    // name is a clipped "Follow"/"Following", which the legibility pass read
    // as 1.08:1 body text.
    if (r.width <= 2 && r.height <= 2 && cs.position === 'absolute' && cs.clipPath !== 'none') continue;
  ...
  const MIN_SIZE = {
    ...
    '.row-thumb': 60,
    '#itube-stage': 200,
    // 60 back when .watch-meta held the identity line AND the actions strip.
    // The two were split across the video — identity above it, controls below
    // in .watch-below — so each block is now one ~34px row and the old floor
    // failed on a correct layout. Both are still floored, so a real collapse
    // (0px, or a row that lost its content) is still caught.
    '.watch-meta': 28,
    '.watch-below': 28,
    '.sidebar': 200,
  };
  ...
  if (grid) {
    ...
    if (cards.length > 1) {
      ...
      if (firstRowRects.length > 1) {
        ...
        // Equal gaps are not enough — zero is equal too, and that is exactly
        // what shipped: `.c` paints an 8px padding box pulled out by
        // margin:-8px, so a 16px column gap left neighbouring cards' hover
        // surfaces flush and hovering one read as a slab spanning two cards.
        // Assert a real break exists, and that it matches the vertical one so
        // the grid's rhythm stays square.
        if (gaps.length) {
          const minGap = Math.min(...gaps);
          if (minGap < 4) {
            report('grid-card-separation', `adjacent card boxes are ${minGap.toFixed(1)}px apart — a card's hover/focus surface must not touch its neighbour's (the grid gap has to exceed the 16px the two overhangs consume)`);
          }
          const belowFirstRow = rects.filter((r) => r.top > firstTop + 2).sort((a, b) => a.top - b.top);
          if (belowFirstRow.length) {
            const vGap = belowFirstRow[0].top - firstRowRects[0].bottom;
            if (Math.abs(vGap - minGap) > 2) {
              report('grid-card-separation', `card gaps not square: ${minGap.toFixed(1)}px horizontally vs ${vGap.toFixed(1)}px vertically`);
            }
          }
        }
      }
  ...
  if (stage) {
    if (!bar) {
      ...
      const stageEl = document.querySelector('#itube-stage');
      for (const id of ['itube-play', 'itube-seek', 'itube-vol', 'itube-theater']) {
        ...
        // Containment is measured against the STAGE, not the bar. The
        // controls used to live in one full-width strip; they are now
        // floating capsules pinned to the stage's corners and centre (the
        // volume capsule top-right, theater/fullscreen bottom-right, the
        // transport cluster dead centre), so "inside #itube-bar" stopped
        // being the invariant. What still matters — and what this catches —
        // is a control drifting outside the player entirely, where it would
        // be unclickable or painted over the page.
        const frameRect = (stageEl || bar).getBoundingClientRect();
        if (cr.left < frameRect.left - 1 || cr.right > frameRect.right + 1 || cr.top < frameRect.top - 1 || cr.bottom > frameRect.bottom + 1) {
          report('player-bar-controls', `#${id} escapes the player stage: control=${JSON.stringify(rectStr(cr))} stage=${JSON.stringify(rectStr(frameRect))}`);
        }
      }
    }
  }
