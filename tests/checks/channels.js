...
async function checkWatchChannelRow(page) {
  ...
  const row = await page.evaluate(() => {
    ...
    // The avatar is the FOLLOW control now, not a link to the channel — the
    // channel name beside it carries that navigation. What has to hold is that
    // the picture still renders inside the control and the control is the
    // subscribe button, so neither the image nor the action was lost.
    const follow = document.querySelector('.watch-channel .watch-subscribe');
    return {
      nameTag: name ? name.tagName : null,
      nameHref: name ? name.getAttribute('href') : null,
      nameText: name ? name.textContent.trim() : '',
      followTag: follow ? follow.tagName : null,
      followHasImg: !!(follow && follow.querySelector('img.watch-avatar')),
      followHasBadge: !!(follow && follow.querySelector('.watch-follow-badge')),
    };
  });
  ...
  if (row.followTag !== 'BUTTON') {
    violations.push({ check: 'watch-avatar-is-follow', detail: `expected the watch avatar to BE the follow control (a <button>), got <${row.followTag}>` });
  }
  if (!row.followHasImg) {
    violations.push({ check: 'watch-avatar-is-follow', detail: 'the follow control contains no <img.watch-avatar> — the channel picture was lost in the restructure' });
  }
  if (!row.followHasBadge) {
    violations.push({ check: 'watch-avatar-is-follow', detail: 'the follow control has no .watch-follow-badge — without it the avatar is an unmarked button and nothing says it follows' });
  }
  return violations;
}
