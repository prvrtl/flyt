#!/usr/bin/env node
// Performance benchmark for Flyt v4, stock YouTube vs the userscript.
//
...
//
// WebKit by default (FLYT_BROWSER=chromium to compare engines), headless
// unless HEADED=1 — the primary user runs Safari, so the perf mandate is
// judged on Safari's engine. The script is injected via addInitScript, i.e. at
// document-start — the same moment a userscript manager runs it.
//
// Headless WebKit reports rAF timestamps at 1ms granularity and pins the frame
// clock near vsync, so `Frame time, median` reads ~14-15ms in BOTH modes and
// cannot show a difference. On this engine the signals that DO separate the
// two are node count, janky-frame count, and worst frame — read those.
'use strict';
...
async function measure(browser, url, withScript) {
  ...
  // `longtask` is Chromium-only. WebKit neither throws nor delivers entries,
  // so an unguarded observer leaves __longTasks at [] and the table then
  // prints "0 long tasks, 0 ms" for BOTH modes — a measurement that looks
  // like a result and is actually just an unimplemented API. Record support
  // explicitly so the reporter can say "unsupported" instead of zero.
  ...
  // Wait for the UI under test to actually be on screen: Flyt's own root when
  ...
  // Scroll the element that actually scrolls in each mode: Flyt scrolls its own
  // `.content` pane; stock YouTube scrolls the document. Scrolling the window in
  // Flyt would move nothing and silently measure an idle page.
  ...
  const longTaskSupported = await page.evaluate(() => !!window.__longTaskSupported);
  await context.close();
...
async function main() {
  ...
  if (runs.stock[0].longTaskSupported) {
    row('Long tasks', (r) => r.longTaskCount);
    row('Long-task time, total', (r) => r.longTaskTotal, (v) => `${Math.round(v)} ms`);
  } else {
    console.log('| Long tasks | n/a | n/a | PerformanceObserver longtask unsupported on this engine |');
  }
